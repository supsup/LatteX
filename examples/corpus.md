...
> **Empirical frontier** over **179 entries** — the tier column is the source of truth in [`corpus.tsv`](../src/test/resources/com/lattex/parse/corpus.tsv), verified against `parse()` by `CorpusParseTest`: `PARSES-NOW` **177**, `NEEDS-PARSER-NODE` **2**, `PARSER-BUG` **0**. The parser fails cleanly (a named `MathSyntaxException`) on the entire not-yet frontier — no crashes.
...
## Cancel family (cancel package strikes)

| LaTeX | Description | Tier |
| --- | --- | --- |
| `\cancel{x}` | up-diagonal strike (/) through the body — \\cancel | `PARSES-NOW` |
| `\bcancel{x}` | down-diagonal strike (\\) — \\bcancel | `PARSES-NOW` |
| `\xcancel{ab}` | both diagonals (an X) — \\xcancel | `PARSES-NOW` |
| `\cancelto{0}{x^2}` | \\cancelto — up strike + arrowhead + target value at the tip | `PARSES-NOW` |
...
| `\newcommand{\half}{\frac{1}{2}} \half + \half` | L8: inline \\newcommand defines and expands (additive-only namespace) | `PARSES-NOW` |
| `\newcommand{\inner}[2]{\langle #1, #2 \rangle} \inner{u}{v}` | L8: #1..#9 argument splicing into the body | `PARSES-NOW` |
| `\def\avg{\frac{#1+#2}{2}} \avg{a}{b}` | L8: \\def subset with arity inferred from the highest #k | `PARSES-NOW` |
