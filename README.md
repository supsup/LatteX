...
metrics (`renderInlineResult` → depth/height in em) so prose math sits on the line. Script
placement consumes the font's OpenType math-kern staircases, so subscripts tuck into a
slanted glyph (V₁, Pₙ) and superscripts clear an overhang (f²) exactly as the font intends.
